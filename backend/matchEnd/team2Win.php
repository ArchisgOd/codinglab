...
if ($resultUsers->num_rows > 0) {
    while ($row = $resultUsers->fetch_assoc()) {
        ...
        WHERE bettomatchesstory.matchID =' ".$matchResult['ID']." 'AND bettomatchesstory.team = '".$matchResult['teamName2']."' AND bettomatchesstory.login = '".$row['login']."'
        GROUP BY users.login), 0) * (SELECT `teamCoefficient2` FROM `matches` WHERE `ID` = '".$matchResult['ID']."')
